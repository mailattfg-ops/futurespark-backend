...
export const scheduleService = {
   ...
   */
  /**
   * Counts for the read-only Display Dashboard.
   *
   * Every value here is an aggregate. Nothing identifying a child, a guardian
   * or a mentor leaves this method - no names, no emails, no phone numbers, no
   * per-family money - because the DISPLAY role exists to put numbers on a
   * screen, and the endpoint is the only one that role can reach. Adding a
   * name to this payload would hand that role personal data.
   */
  async getDisplayMetrics(callerRole?: string) {
    if (!callerRole || !['ADMIN', 'DISPLAY'].includes(callerRole)) {
      throw new AppError('Not permitted', HTTP_STATUS.FORBIDDEN);
    }
    ...
    const [
      ...
      db.student.count({ where: { status: 'ACTIVE', isActive: true } }),
      db.program.count(),
      db.user.count({ where: { isActive: true, role: { name: 'TEACHER' } } }),
      db.scheduledClass.count({ where: { status: 'SCHEDULED', startTime: { gte: startOfToday, lt: endOfToday } } }),
      db.scheduledClass.count({ where: { status: 'SCHEDULED', startTime: { gte: startOfToday, lt: endOfWeek } } }),
      db.scheduledClass.count({ where: { status: 'COMPLETED' } }),
      db.scheduledClass.count({ where: { status: 'COMPLETED', startTime: { gte: thirtyDaysAgo } } }),
      // Start times only - enough to bucket by month, and identifies nobody.
      db.scheduledClass.findMany({
        where: { status: 'COMPLETED', startTime: { gte: sevenMonthsAgo } },
        select: { startTime: true },
      }),
      db.enrollment.groupBy({ by: ['programId'], _count: { _all: true } }),
      db.session.groupBy({ by: ['programId'], _count: { _all: true } }),
    ]);

    const curriculumSize = new Map(sessionsByProgram.map((s) => [s.programId, s._count._all]));
    const curriculumTotal = enrolmentsByProgram.reduce(
      (n, e) => n + e._count._all * (curriculumSize.get(e.programId) ?? 0),
      0
    );
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      months.push({
        ...
        count: recentCompleted.filter((c) => c.startTime >= from && c.startTime < to).length,
      });
    }
    ...
    return {
      ...
      deliveredTotal,
      delivered30d,
      // Share of the curriculum enrolled for that has actually been taught.
      completionRate: curriculumTotal ? Math.min(100, Math.round((deliveredTotal / curriculumTotal) * 100)) : 0,
      months,
