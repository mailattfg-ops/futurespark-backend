...
import { drawSessionReport } from './report-design';
import { buildReportDocument, type ReportCurriculum } from './report-document';
...
export interface ReportContext {
  ...
  /**
   * Everything the approved design shows that the analysis has no view of:
   * the arc, the topic map, the outcomes, the activities and what comes next.
   * Absent fields fall back to the analysis, so a class whose curriculum has
   * not been filled in yet still renders a complete page.
   */
...
export const renderSessionReportPdf = async (
  ...
  if (report) {
    /* The approved two-page design.
     *
     * Curriculum content and measured content are assembled separately and
     * then drawn — report-document.ts says which half owns what, and why the
     * curriculum wins wherever both could answer. */
      ...
      buildReportDocument(
        {
          ...
          dateLabel: ctx.classDate,
          timeLabel: ctx.classTime,
